...
import { SeachFoodAPI } from '@/services/List';
import type { ListItem } from '@/types/List';
import type { PageParams } from '@/types/global';
import { ref } from 'vue';

const keyword = ref('');
const SearchFoodList = ref<ListItem[]>([]);
...
const getListData = async (searchKeyword: string) => {
  ...
  //读取搜索的第一个页面
  const res3 = await SeachFoodAPI(searchKeyword, pageParams.page.toString())
  //放入列表
  SearchFoodList.value.push(...res3.datas)
  if (SearchFoodList.value.length === 0) {
    finish.value = true
  ...
  if (pageParams.page < Number(res3.data.totalPages)) {
    // 页码累加
    pageParams.page++;
    console.log('当前读取的美食搜索页数:', pageParams.page.toString());
  } else {
    finish.value = true
  }
}
...
<template>
  ...
  >
    <view v-for="item in SearchFoodList" :key="item.name">
      <List-Column
        :picSrc="item.picture"
        :star-name="item.name"
        :rating="item.rank"
